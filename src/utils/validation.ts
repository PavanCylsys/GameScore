...
export function isValidDob(dob: string): boolean {
  ...
  const isDmy = DOB_REGEX_DMY.test(trimmed);
...
export function getDobValidationError(dob: string): string | undefined {
  ...
  const trimmed = dob.trim();
  if (!DOB_REGEX_ISO.test(trimmed) && !DOB_REGEX_DMY.test(trimmed)) {
    return "Date of birth must be in format YYYY-MM-DD or DD/MM/YYYY";
  }
  const d = DOB_REGEX_DMY.test(trimmed) ? parseDDMMYYYY(trimmed) : new Date(trimmed);
...
/** Normalize DOB string (YYYY-MM-DD or DD/MM/YYYY) to YYYY-MM-DD for DB. */
export function normalizeDobToIso(dob: string): string {
  const trimmed = String(dob).trim();
  if (DOB_REGEX_DMY.test(trimmed)) {
